...
def main():
    hometask = HomeTask()
    public_tweets = hometask.get_tweets()
    for tweet in public_tweets:
        if hometask.hometask_exclude(tweet):
            pass
        else:
            if hometask.check_task(tweet): # set taskをする
                reply = set_task_and_reply(tweet)
            elif hometask.check_task_history(tweet): # hometask
                reply = set_task_history_and_reply(tweet)
